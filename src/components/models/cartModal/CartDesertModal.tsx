...
export interface ICartDesertModalProps {}

export default function CartDesertModal(props: ICartDesertModalProps) {
  const {desertCartModal, selectedItem } = UseSelector((state) => state.food);
...
const whatWeWantToAddToCart = {
  ...
  return (
    <div>
      <Modal
        open={desertCartModal}
        onClose={() => dispatch(setHandleClose("desertCart"))}
      >
          ...
          <Box className="flex flex-col w-full h-full p-6">
            ...
            <Box className="flex flex-col w-full border-b border-gray-400 pb-8">
              <Box className="flex items-center justify-between mb-2">
                ...
                <CloseIcon
                  onClick={() => dispatch(setHandleClose("desertCart"))}
                  className="text-[29px]"
                />
              </Box>
            ...
            <Box className="flex sm:items-center w-full flex-col sm:flex-row justify-center sm:justify-normal">
                ...
                <Button variant="contained" className="btnColor text-white w-full sm:ml-5 p-3 rounded-3xl hover:bg-green-700" onClick={()=>{
                        ...
                        dispatch(setHandleClose("desertCart"))
                        setTextArea("")
                        setCount(1)
