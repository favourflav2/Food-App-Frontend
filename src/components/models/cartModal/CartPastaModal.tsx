...
export interface ICartPastaModalProps {}

export default function CartPastaModal(props: ICartPastaModalProps) {
  const { pastaCartModal, selectedItem } = UseSelector((state) => state.food);
...
const whatWeWantToAddToCartUpdate = {
  ...
  return (
    <div>
      <Modal
        open={pastaCartModal}
        onClose={() => dispatch(setHandleClose("pastaCart"))}
      >
          ...
          <Box className="flex flex-col w-full h-full p-6">
            ...
            <Box className="flex flex-col w-full border-b border-gray-400 pb-8">
              <Box className="flex items-center justify-between mb-2">
                ...
                <CloseIcon
                  onClick={() => dispatch(setHandleClose("pastaCart"))}
                  className="text-[29px]"
                />
              </Box>
