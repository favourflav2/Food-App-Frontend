...
export interface ISaladModalProps {}

export default function SaladModal(props: ISaladModalProps) {
  const { saladModal, selectedItem } = UseSelector((state) => state.food);
  const dispatch = Dispatch();
  //console.log(selectedItem);
  const [Price, setPrice] = React.useState<number>(Number(selectedItem?.price));
  const [state, setState] = React.useState<string>("Full");
...
const whatWeWantToAddToCart = {
  ...
  return (
    <div>
      <Modal
        open={saladModal}
        onClose={() => dispatch(setHandleClose("salad"))}
      >
          ...
          <Box className="flex flex-col w-full h-full p-6">
            ...
            <Box className="flex flex-col w-full border-b border-gray-400 pb-8">
              <Box className="flex items-center justify-between mb-2">
                ...
                <CloseIcon
                  onClick={() => dispatch(setHandleClose("salad"))}
                  className="text-[29px]"
                />
              </Box>
